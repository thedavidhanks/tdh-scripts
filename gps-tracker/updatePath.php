...
function getMQroute($point1, $point2){
    ...
    $apiKey = "WXcirP8j0yMOAlAhkD5aaowlUnskmEpI";
...
function addPointToPath($newLatLong, $trip) {
    ...
    $mqResult = json_decode(getMQroute($lastLatLong, $newLatLong));
            ...
            if($query_sel->rowCount() > 0){
                foreach ($query_sel as $row){
                    ...
                    if(!empty($previousLatLong)){
                        ...
                        $mqResult = json_decode(getMQroute($previousLatLong, $currentLatLong));
                        ...
                        //If the points not the same nake a new leg
                        $distance = $mqResult->{"route"}->{"distance"};
                        if($distance > 0){
                            $tripLeg_id += 10;
                            $newLeg = new TripLeg();
